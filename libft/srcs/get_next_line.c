...
/*   Updated: 2016/02/03 19:49:04 by ibouchla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_next_line(int const fd, char **line)
{
	...
	*line = (char *)ft_memalloc(1);
	if ((ft_traiting_value_rest(rest[fd], buf, line)) == 1)
		return (1);
	while (((rd = read(fd, buf, BUFF_SIZE)) > 0) && ((ft_check(buf, 0)) == 0))
		ft_storage_in_line(buf, line, rd, 0);
	if (rd == (-1))
		return (-1);
	if ((rd == 0 && (!rest[fd] || rest[fd][0] == '\0')) && *line[0] == '\0')
		return (0);
	if (rest[fd] && rest[fd][0] != '\0')
		ft_storage_in_line(rest[fd], line, rd, (ft_check(rest[fd], 0)));
	if ((ft_check(buf, 0)) == 1)
	{
		ft_storage_in_line(buf, line, rd, 1);
		rest[fd] = ft_storage_rest(buf, rd);
	}
